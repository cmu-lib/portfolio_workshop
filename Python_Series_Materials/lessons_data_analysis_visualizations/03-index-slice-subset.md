...
`iloc` method. For example, we can select the first three rows 
with the month, day and year (columns 2, 3
...
```python
# Select all columns for the first three rows
surveys_df.iloc[0:3, :]
...
Now let's explore what the `loc` method does
...
using `iloc`, and direct indexing will raise a `KeyError` if a single integer or list is used; the
...
2. While `iloc` uses integers as indices and slices accordingly, `loc` works with labels.
   ...
   Check also the difference between `surveys_df.loc[0:4]` and `surveys_df.iloc[0:4]`. Then finally, try `surveys_df.loc[0:4, 'month':'year']`, to see how to use a range of column labels.
...
### Take Home Challenge - Putting it all together
