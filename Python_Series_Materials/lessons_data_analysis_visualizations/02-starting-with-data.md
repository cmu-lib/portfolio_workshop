...
from Ernst et al.
[Long-term monitoring and experimental manipulation of a Chihuahuan Desert ecosystem near Portal, Arizona, USA][ernst].
...
[Download surveys.csv dataset][figshare-ndownloader]
...
files, see the [Data Organization in Spreadsheets][spreadsheet-lesson5] lesson.
...
5. `surveys_df.columns` provides the names of the columns in the DataFrame.
6. `surveys_df.shape` provides the dimensions of the DataFrame as a tuple 
   in `(r,c)` format,
   where `r` is the number of rows and `c` the number of columns.
7. `surveys_df.head()` returns the first 5 lines of the DataFrame, 
...
8. `surveys_df.tail()` will display the last 5 lines, 
...
### Solution to Take Home Challenge
